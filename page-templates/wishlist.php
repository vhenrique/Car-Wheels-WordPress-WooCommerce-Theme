    ...
    <table class="shop_table cart wishlist_table" cellspacing="0">
        <thead>
            <tr>
                ...
                <th class="product-name"><span class="nobr"><?php _e( 'Product Name', 'commercegurus' ) ?></span></th>
                <?php if ( get_option( 'yith_wcwl_price_show' ) == 'yes' ) : ?><th class="product-price"><span class="nobr"><?php _e( 'Unit Price', 'commercegurus' ) ?></span></th><?php endif ?>
                <?php if ( get_option( 'yith_wcwl_stock_show' ) == 'yes' ) : ?><th><span class="nobr"><?php _e( 'Stock Status', 'commercegurus' ) ?></span></th><?php endif ?>
        ...
        <tbody>
            ...
            if ( count( $wishlist ) > 0 ) :
                foreach ( $wishlist as $values ) :
                    ...
                    if ( $product_obj !== false && $product_obj->exists() ) :
                        ...
                        <tr id="yith-wcwl-row-<?php echo $values['ID'] ?>">
                            <td class="product-remove"><div><a href="javascript:void(0)" onclick="remove_item_from_wishlist( '<?php echo esc_url( $yith_wcwl->get_remove_url( $values['ID'] ) ) ?>', 'yith-wcwl-row-<?php echo $values['ID'] ?>' );" class="remove" title="<?php _e( 'Remove this product', 'commercegurus' ) ?>">&times;</a></td>
                            ...
                            <?php if ( get_option( 'yith_wcwl_price_show' ) == 'yes' ) : ?>
                                <td class="product-price">
                                    ...
                                    if ( $product_obj->price != '0' ) {
                                        ...
                                        echo apply_filters( 'yith_free_text', __( 'Free!', 'commercegurus' ) );
                                    }
                                    ?>
                                </td>
                            <?php endif ?>
                            <?php if ( get_option( 'yith_wcwl_stock_show' ) == 'yes' ) : ?>
                                <td class="product-stock-status">
                                    ...
                                    if ( $stock_status == 'out-of-stock' ) {
                                        $stock_status = "Out";
                                        echo '<span class="wishlist-out-of-stock">' . __( 'Out of Stock', 'commercegurus' ) . '</span>';
                                    } else {
                                        $stock_status = "In";
                                        echo '<span class="wishlist-in-stock">' . __( 'In Stock', 'commercegurus' ) . '</span>';
                                    }
                                    ?>
                                </td>
                            <?php endif ?>
                ...
                ?>
                <tr>
                    <td colspan="6" class="wishlist-empty"><?php _e( 'No products were added to the wishlist', 'commercegurus' ) ?></td>
                </tr>
            <?php
            endif;
